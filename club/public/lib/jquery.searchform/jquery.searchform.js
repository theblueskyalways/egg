(function ($) {
		...
		settings.forEach(function (v, i) {
			...
			case 'dateGroup':
				...
				if (Array.isArray(v.data)) {
					...
					if (v.default) {
						html += '<input type="radio" name="jquery-searchform-radio-' + i + '" value="0" title="' + v.default + '">';
					}
					v.data.forEach(function (v) {
						html += '<input type="radio" name="jquery-searchform-radio-' + i + '" value="' + v.id + '" title="' + v.name + '">';
					});
					html +=
	...
	function search($items, page) {
		var params = [], text = null, date = null, dateGroup = null;
		$items.each(function () {
			var $item = $(this);
			if ($item.data('type') == 'input') {
