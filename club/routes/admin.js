...
router.post('/login', async (req, res) => {
    try {
        ...
        if (user.length > 0) {
            ...
            let url = '/main';
            if(user.role === consts.ADMIN) url='/club';
            if(user.role === consts.USER) url='/main';
            res.json({
                code: code.OK,
                user,
                url,
...
router.post('/register', async (req, res) => {
    try {
        ...
        if (func.empty(phone) || func.empty(password)) return res.json(code.message(code.InvalidParam));
        let user = await mysql.query('SELECT * FROM v_user WHERE phone=?', [
                phone
            ]);
            
        if (user.length == 0) {
            let result = await mysql.query('INSERT INTO v_user(name,role,phone,password,is_delete,ctime) VALUES(?,?,?,?,?,?)', [
...
router.post('/user/list', async (req, res) => {
    try {
        ...
        let role = array.assoc(consts.role,'id'); 
        list = list.map(v=>({
            ...v,
...
router.post('/user/add', async (req, res) => {
    try {
        ...
        let name = func.trim(req.body.name || ''),
...
router.post('/user/editor', async (req, res) => {
    try {
        ...
        let id = func.toInt(req.body.id || 0),
            ...
            name = func.trim(req.body.name, '');
        if (role <= 0 || func.empty(phone) || id <= 0 || func.empty(name))
            return res.json({code: code.InvalidParam, msg: '参数错误'});
...
router.post('/club/add', async (req, res) => {
    try {
        ...
        if (id <= 0) {
            let result = await mysql.query('INSERT INTO v_club(name,clubDesc,imageUrl,payQRcode,ctime,is_delete) VALUES(?,?,?,?,?,?)', [
...
router.post('/active/listpage', async (req, res) => {
    try {
        ...
        values = values.concat([(page - 1) * consts.PAGE_SIZE, consts.PAGE_SIZE + 1]);
        let list = await mysql.query(sql, values);
        let clubList = await mysql.query(`SELECT id,name FROM v_club WHERE is_delete=0`);
        let clubList_ = array.assoc(clubList,'id','all');
        list = list.map(v=>({
            ...v,
...
router.post('/apply/listpage', async (req, res) => {
    try {
        ...
        if (!func.empty(name)) {
            ...
            let user = await mysql.query(userSql,[userValues]);
